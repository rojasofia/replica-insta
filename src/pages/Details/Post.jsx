...
const Post = ({ post, currentUser }) => {
  ...
  const [liked, setLiked] = useState(
    currentUser && currentUser.likesStore.includes(post.id)
  ); // Verifica si el usuario actual ya dio "me gusta" al post
  ...
  useEffect(() => {
    fetch(`${URL_API}posts/${post.id}/likes`)
      ...
      .then((likes) => {
        ...
        setLiked(
          currentUser && likes.find((like) => like.userId === currentUser.id)
        );
      })
  ...
  const handleLike = () => {
    ...
    fetch(url, {
      ...
      .catch((error) =>
        console.error("Error al añadir o eliminar like:", error)
      );
  };
            ...
            <div className="user-info">
              ...
              <div className="post-stats">
                <FontAwesomeIcon
                  ...
                  icon={faHeart}
                  onClick={handleLike}
                  color={liked ? "#eb4d4d" : "#c0a7caeb"}
                />{" "}
                <label>{likesCount}</label>
                <FontAwesomeIcon
                  className="Iconos"
                  icon={faComment}
                  style={{ color: "#c0a7caeb" }}
                />{" "}
